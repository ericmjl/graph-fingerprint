...
from convnet import GraphInputLayer, GraphConvLayer, FingerprintLayer, LinearRegressionLayer
...
n_graphs = 200
graphs = [make_random_graph(sample(all_nodes, choice(n_nodes)), choice(n_nodes), features_dict) for i in range(n_graphs)]
len(graphs)

input_shape = (1, 10)
inputs = GraphInputLayer(input_shape).forward_pass(graphs)
node_indices, nbr_indices, graph_indices = GraphInputLayer(input_shape).graph_indices(graphs)
print('Node indices:')
print(node_indices)
...
def predict(wb_vect, wb_unflattener, inputs, layers, node_indices, nbr_indices, graph_indices):
    curr_inputs = inputs
    start = time()
    ...
    for i, layer in enumerate(layers):
        ...
        curr_inputs = layer.forward_pass(wb, curr_inputs, node_indices, nbr_indices, graph_indices)
    return curr_inputs

predict(*wb_all.flattened(), inputs, layers, node_indices, nbr_indices, graph_indices)

graph_scores = np.array([float(score(g)) for g in graphs])
print(graph_scores.dtype)

def train_loss(wb_vect, wb_unflattener, inputs, layers, node_indices, nbr_indices, graph_indices, graphs, graph_scores):
    """
    Training loss is MSE.
    """
    preds = predict(wb_vect, wb_unflattener, inputs, layers, node_indices, nbr_indices, graph_indices)
...
# train_loss(*wb_all.flattened(), inputs, layers, node_indices, nbr_indices, graph_indices, graphs, graph_scores)

gradfunc = grad(train_loss)
# gradfunc(*wb_all.flattened(), inputs, layers, node_indices, nbr_indices, graph_indices, graphs, graph_scores)

wb_vect, wb_unflattener = wb_all.flattened()


def sgd(gradfunc, wb_vect, wb_unflattener, inputs, layers, node_indices, nbr_indices, graph_indices, graphs, graph_scores, callback=None, num_iters=200, step_size=0.1, mass=0.9):
    """
    ...
    for i in range(num_iters):
        ...
        g = gradfunc(wb_vect, wb_unflattener, inputs, layers, node_indices, nbr_indices, graph_indices, graphs, graph_scores)
        velocity = mass * velocity - (1.0 - mass) * g
        wb_vect += step_size * velocity
        print('Training Loss: ')
        print(train_loss(wb_vect, wb_unflattener, inputs, layers, node_indices, nbr_indices, graph_indices, graphs, graph_scores))
        end = time()
        print('Time: {0}'.format(end - start))
        print('')
    # return wb_vect, wb_unflattener

sgd(gradfunc, wb_vect, wb_unflattener, inputs, layers, node_indices, nbr_indices, graph_indices, graphs, graph_scores, num_iters=1000, step_size=0.0001)
